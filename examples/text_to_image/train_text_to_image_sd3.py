...
def log_validation(transformer, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        transformer = SD3Transformer2DModel.from_pretrained(args.output_dir, torch_dtype=weight_dtype)
        ...
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                tracker.writer.add_image(
                    "transformer conditioning", np.asarray([validation_image]), step, dataformats="NHWC"
                )
            ...
            for log in image_logs:
                ...
                formatted_images.append(wandb.Image(validation_image, caption="transformer conditioning"))
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    model_description = f"""
# SD3 transformer-{repo_id}

These are transformer weights trained on {base_model} with new type of conditioning.
The weights were trained using [transformer](https://github.com/lllyasviel/transformer) with the [SD3 diffusers trainer](https://github.com/huggingface/diffusers/blob/main/examples/transformer/README_sd3.md).
{img_str}
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a transformer training script.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        " If not specified transformer weights are initialized from unet.",
    )
    parser.add_argument(
        "--num_extra_conditioning_channels",
        type=int,
        default=0,
        help="Number of extra conditioning channels for transformer.",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        help="The column of the dataset containing the transformer conditioning image.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of paths to the transformer conditioning image be evaluated every `--validation_steps`"
    ...
    if args.resolution % 8 != 0:
        raise ValueError(
            "`--resolution` must be divisible by 8 for consistently sized encoded images between the VAE and the transformer encoder."
        )
...
def main(args):
    ...
    if args.transformer_model_name_or_path:
        logger.info("Loading existing transformer weights")
        transformer = SD3Transformer2DModel.from_pretrained(args.transformer_model_name_or_path)
    else:
        logger.info("Initializing transformer weights from transformer")
        transformer = SD3Transformer2DModel.from_transformer(
    ...
    def unwrap_model(model):
        model = accelerator.unwrap_model(model) #移除模型在并行训练中可能被包装的 DataParallel 或 DistributedDataParallel 等包装器，返回未包装的原始模型
    ...
    if unwrap_model(transformer).dtype != torch.float32:
        raise ValueError(
            f"Transformer loaded as datatype {unwrap_model(transformer).dtype}. {low_precision_error_string}"
        )
    ...
    del text_encoder_one, text_encoder_two, text_encoder_three, compute_embeddings_fn
    del tokenizer_one, tokenizer_two, tokenizer_three, vae
    free_memory()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                noisy_model_input = (1.0 - sigmas) * model_input + sigmas * noise  #疑问：这个是加噪声吗?
